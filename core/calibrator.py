...
class HestonCalibrator:
    ...
    def _mom_calibrate(
        ...
        #
        # KNOWN LIMITATION — this number is barely an estimate. Measured against
        # series generated by THIS repo's simulator at a known rho, it returns
        # roughly the same small value whatever the truth:
        #
        #     true -0.90 -> -0.04 | true -0.70 -> -0.04 | true -0.40 -> -0.03
        #     true  0.00 -> +0.00 | true +0.40 -> +0.04
        #
        # The sign survives, the magnitude does not, so every simulation runs with
        # almost no leverage and the paths carry far less downside skew than the
        # underlying really has. On a worst-of note that understates the knock-in.
        # Do not read the calibration table's rho as a measurement of leverage.
        #
        # The signal is in the data, just not at the DAILY horizon: 21-day
        # aggregated skew on the same series separates cleanly (-0.68 at rho=-0.9
        # through +0.39 at rho=+0.4). A replacement should fit there. Pinned by
        # tests/test_calibrator.py, which is written to FAIL when this is fixed.
        #
        # An earlier review claimed the cause was an off-by-one lag here. It is
        # not: a lag scan over real SPY/AAPL/MSFT recovers no leverage at any lag,
        # and the proposed shift makes rho POSITIVE. The daily horizon is the
        # problem, not the alignment.

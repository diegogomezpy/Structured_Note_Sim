...
# Leverage prior. rho is NOT identifiable from five years of daily prices, and
# the app used to ship whatever the daily estimator returned — which is ~-0.04
# regardless of the truth (see _estimate_leverage below). That asserts "this
# underlying has no leverage effect", the one value we can be confident is wrong
# for equities.
#
# The identification is not a tuning problem, it is a sample-size one. Fitting
# rho to the 21-day aggregated skew (where the signal genuinely lives) has a
# slope of 0.595 skew per unit rho, but a single 5-year series estimates that
# skew with a standard deviation of ~0.6 — an implied uncertainty of +-1.05 on
# rho, wider than its entire valid range. Averaging many independent series
# separates cleanly; you only ever have one. The information that would pin rho
# is in the OPTION surface, which this app does not consume.
#
# So: use a prior. -0.6 is the middle of the equity leverage range reported
# across the literature. Measured on a representative 2y worst-of, moving from
# the estimator's -0.04 to -0.6 takes P(knock-in) from 3.78% to 5.75% — a 52%
# relative increase, in the direction the estimator was understating.
# Pass rho_prior=None to use the raw estimate instead.
LEVERAGE_PRIOR = -0.6
...
class HestonCalibrator:
    ...
    def __init__(
        ...
        rho_prior:   Optional[float]         = LEVERAGE_PRIOR,
    ):
    ...
    def _mom_calibrate(
        ...
        #
        # This estimate is a DIAGNOSTIC ONLY — `rho_prior` is what gets simulated.
        # Measured against series generated at a known rho it is flat (-0.04 at a
        # true -0.90, -0.03 at -0.40, +0.04 at +0.40), and the shortfall is not
        # fixable at this sample size: see LEVERAGE_PRIOR at the top of the file
        # for the identification argument and the measured cost.
        #
        # An earlier review blamed an off-by-one lag here. It is not that: a lag
        # scan over real SPY/AAPL/MSFT recovers no leverage at any lag, and the
        # proposed shift makes rho POSITIVE. The daily horizon is the problem.
        # We therefore align lr1[:-1] with d_rv (both length len(rv_clean)-1).
        min_len = min(len(lr1) - 1, len(d_rv))
        if min_len > 10:
            rho_est = float(np.corrcoef(lr1[:min_len], d_rv[:min_len])[0, 1])
        else:
            rho_est = -0.5  # fallback
        # The estimate is kept for the diagnostics table but is NOT what gets
        # simulated: it carries no usable information (see LEVERAGE_PRIOR).
        rho = rho_est if self.rho_prior is None else float(self.rho_prior)

/* Note-type presets. Selecting a type stores an explicit `note_type` on the note
   ...
   with its own good payoff + menus — reintroduce one at a time):
     - reverse_conv    — reverse convertible: guaranteed coupon (no coupon barrier),
                         no memory. Rides the Autocall waterfall (coupon_barrier=0).
   ...
   these types still load and price via the Autocall waterfall; they're just not
   offered in the UI. See CLAUDE.md "Note types". */
import type { NoteTerms } from '../api/types'

export type NoteType = 'autocall' | 'reverse_conv' | 'growth_autocall' | 'participation' | 'custom'
...
export const NOTE_TYPES: NoteType[] = ['autocall', 'participation']

/** The note's structure type for the UI. Only Autocall and Participation are
    surfaced; any parked/legacy type collapses to Autocall (it still prices from its
    stored fields via the Autocall waterfall — this only affects the picker/menu). */
...
export function applyPreset(t: NoteTerms, type: NoteType): NoteTerms {
  ...
  switch (type) {
    case 'participation':
      // Maturity-only payoff — neutralise the entire Autocall waterfall.

...
const VIEW_W = 720
const X0 = 40
const X1 = 414        // axis right end / last node — leaves a wide label gutter
const VIEW_H = 150
const Y_TOP = 26
const Y_BOT = 126
const DOMAIN = 1.22   // level mapped to the top of the plot
const LABEL_X = X1 + 24

const mapY = (level: number) => Y_BOT - (Math.min(Math.max(level, 0), DOMAIN) / DOMAIN) * (Y_BOT - Y_TOP)
const mapX = (frac: number) => X0 + frac * (X1 - X0)

/** Greedily separate label y-positions by at least `gap`, keeping them inside
    [minY, maxY]. Returns adjusted y's in the original order — so a barrier label
    can be drawn clear of its neighbours with a leader line back to the line. */
...
/** Live schematic of the note: observation timeline + autocall window + the
    barrier reference lines, with a descriptive key beneath. Pure function of the
    terms — no simulation. */
export default function NoteTimeline({ terms }: { terms: NoteTerms }) {
  ...
  // ── right-gutter barrier labels (de-collided) ───────────────────────────────
  const entries: GutterEntry[] = [{
    target: lineY, color: 'var(--accent-text)', name: t('autocall_barrier'),
    value: stepped ? `${pct(acLevel, 0)} → ${pct(minAc, 0)}` : pct(acLevel, 0),
  }]
  if (barriersEqual) {
    entries.push({ target: mapY(cpLevel), color: 'var(--accent)', name: `${t('coupon_barrier')} · ${t('knock_in_barrier')}`, value: pct(cpLevel, 0) })
  } else {
    entries.push({ target: mapY(cpLevel), color: 'var(--accent)', name: t('coupon_barrier'), value: pct(cpLevel, 0) })
    entries.push({ target: mapY(kiLevel), color: 'var(--red)', name: t('knock_in_barrier'), value: pct(kiLevel, 0) })
  }
  if (osLevel != null) entries.push({ target: mapY(osLevel), color: '#0d9488', name: t('one_star'), value: pct(osLevel, 0) })
  ...
  const legend: { color: string; text: string }[] = [
    ...
    { color: 'var(--accent)', text: t('diag_lgd_coupon', { cpn: pct(terms.coupon_pa, 1), lvl: pct(cpLevel, 0), mem: terms.memory ? t('diag_lgd_coupon_mem') : '' }) },
    { color: 'var(--red)', text: t('diag_lgd_knockin', { lvl: pct(kiLevel, 0) }) },
  ]
  if (osLevel != null) legend.push({ color: '#0d9488', text: t('diag_lgd_onestar', { lvl: pct(osLevel, 0) }) })
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} width="100%" style={{ display: 'block', fontFamily: 'IBM Plex Sans, sans-serif' }}
        ...
        {/* autocall window band + ledge */}
        <rect x={acX} y={lineY - 16} width={X1 - acX} height={32} rx={9} fill="var(--accent-weak)" />
        <line x1={acX + 9} y1={lineY - 16} x2={X1} y2={lineY - 16} stroke="var(--accent)" strokeWidth="1" opacity="0.3" strokeLinecap="round" />

        {/* barrier reference lines */}
        <line x1={X0} y1={mapY(cpLevel)} x2={X1} y2={mapY(cpLevel)} stroke="var(--accent)" strokeWidth="1.2" strokeDasharray="4 3" strokeLinecap="round" opacity="0.7" />
        {!barriersEqual && (
          <line x1={X0} y1={mapY(kiLevel)} x2={X1} y2={mapY(kiLevel)} stroke="var(--red)" strokeWidth="1.2" strokeDasharray="4 3" strokeLinecap="round" opacity="0.8" />
        )}
        {osLevel != null && (
          <line x1={X0} y1={mapY(osLevel)} x2={X1} y2={mapY(osLevel)} stroke="#0d9488" strokeWidth="1.2" strokeDasharray="4 3" strokeLinecap="round" opacity="0.75" />
        )}
        {stepPath && (
          <path d={stepPath} fill="none" stroke="var(--text-muted)" strokeWidth="1.4" strokeDasharray="4 3" strokeLinecap="round" opacity="0.85" />
        )}

        {/* gutter labels with leader lines */}
        {entries.map((e, i) => (
          <g key={i}>
            <path d={`M ${X1 + 4} ${e.target.toFixed(1)} L ${LABEL_X - 6} ${adjY[i].toFixed(1)}`}
                  fill="none" stroke={e.color} strokeWidth="1" strokeDasharray="3 2" opacity="0.7" />
        ...
        {fracs.map((f, i) => {
          ...
          return (
            <g key={k}>
              ...
              {isMat ? (
                <text x={x + 2} y={lineY - 13} fontSize="10" fontWeight={500} fill="var(--text-muted)" textAnchor="end">{t('maturity_short')}</text>
              ) : showDotLabels && (
                <text x={x} y={lineY - 13} fontSize="9" fill="var(--text-faint)" textAnchor="middle">P{k}</text>
              )}
              {/* first-call marker: a small upward triangle (the legend names it) */}
              {k === start && !isMat && (
                <path d={`M ${x} ${lineY + 9} l 4 6 h -8 z`} fill="var(--accent)" opacity="0.9" />
              )}
            </g>
          )
        })}
      ...
      {/* descriptive key */}
      <div style={{ marginTop: 4, paddingTop: 10, borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 7 }}>
        ...
        {legend.map((it, i) => (
          <div key={i} style={{ display: 'flex', gap: 9, alignItems: 'flex-start', fontSize: 12, lineHeight: 1.5 }}>
            <span style={{ width: 9, height: 9, borderRadius: 3, background: it.color, marginTop: 4, flexShrink: 0 }} />

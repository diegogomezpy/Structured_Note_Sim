...
def _share_blockers(terms_a: NoteTerms, terms_b: NoteTerms) -> list[str]:
...
def _can_share_paths(terms_a: NoteTerms, terms_b: NoteTerms) -> bool:
    """Whether an A/B compare can share one simulation — see _share_blockers."""
    return not _share_blockers(terms_a, terms_b)
...
def _outcome_buckets(note: dict, terms: NoteTerms) -> tuple[np.ndarray, list[str]]:
...
def _paired_stats(note_a: dict, note_b: dict, terms_a: NoteTerms, terms_b: NoteTerms) -> dict:
    ...
    code_a, labels_a = _outcome_buckets(note_a, terms_a)
    code_b, labels_b = _outcome_buckets(note_b, terms_b)
    k = len(labels_a)
...
def _metric_samples(note: dict, key: str) -> np.ndarray | None:
...
def _compare_diff(sum_a: dict, sum_b: dict, terms_a: NoteTerms, terms_b: NoteTerms,
    ...
    def _se(key: str) -> float | None:
        if note_a is None or note_b is None:
            return None
        xa, xb = _metric_samples(note_a, key), _metric_samples(note_b, key)
    ...
    for k in keys:
        ...
        rows.append({"key": k, "a": a, "b": b, "delta": d, "se": _f(_se(k))})

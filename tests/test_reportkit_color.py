...
def test_adapter_still_exposes_the_old_private_names():
    """`app/pdf_report.py` re-exports these under their original spellings so no
    call site moved in the extraction."""
    import pdf_report as P

    assert P._remap_color is remap_color
    assert P._parse_rgb is parse_rgb
    assert P._rgb_to_hue is rgb_to_hue
